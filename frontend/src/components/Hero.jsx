    ...
    <section className="relative min-h-screen flex items-center justify-center pt-24 pb-12 px-6 overflow-hidden">
      <div className="relative z-10 w-full max-w-6xl mx-auto">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 md:grid-rows-[auto_auto] gap-4 md:gap-6">
          ...
          <TiltCard className="md:col-span-2 row-span-1 glass-strong rounded-[2rem] p-8 md:p-12 border border-zinc-800/60 overflow-hidden relative">
            <div className="absolute inset-0 bg-gradient-to-br from-[var(--accent)]/5 to-transparent pointer-events-none" />
            <motion.div
              initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.2 }}
              className="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-zinc-900/80 border border-zinc-700/50 mb-8"
            >
              <Sparkles size={14} className="text-[var(--accent)]" />
              <span className="text-xs font-mono text-zinc-300 uppercase tracking-widest">{heroData.tagline}</span>
            </motion.div>

            <h1 className="text-5xl sm:text-6xl lg:text-7xl font-extrabold font-display leading-[1.1] tracking-tight mb-6">
              <StaggeredText text={`I'm ${heroData.name},`} className="text-zinc-100 block" />
              <motion.span 
          ...
          <TiltCard className="md:col-span-1 row-span-1 glass-strong rounded-[2rem] p-4 border border-zinc-800/60 flex items-center justify-center relative overflow-hidden group">
            ...
            <div className="relative w-full h-full aspect-square rounded-[1.5rem] overflow-hidden border border-zinc-700/50">
              <img 
                src="/profile.png" 
                alt={heroData.name} 
          ...
          <motion.div 
            ...
            className="md:col-span-1 glass rounded-[2rem] p-6 border border-zinc-800/60 flex flex-col justify-center gap-4 relative overflow-hidden"
          >
            ...
            <a href="/resume.pdf" target="_blank" rel="noopener noreferrer" className="w-full group px-6 py-4 rounded-xl bg-zinc-900 border border-zinc-800 hover:border-zinc-700 hover:translate-x-1.5 transition-all flex items-center justify-between text-zinc-100">
              <span className="font-semibold text-sm">Download Resume</span>
              <FileText size={18} className="text-zinc-500 group-hover:text-zinc-100 transition-colors" />
            </a>
          </motion.div>

          {/* Location & Status Tile */}
          <TiltCard className="md:col-span-1 glass rounded-[2rem] p-8 border border-zinc-800/60 flex flex-col justify-center relative overflow-hidden group">
            <div className="absolute top-0 right-0 p-6 opacity-10 group-hover:opacity-30 transition-opacity">
              <MapPin size={64} />
            </div>
            <h3 className="text-zinc-400 text-sm font-medium mb-1">Current Base</h3>
            <p className="text-2xl font-bold font-display text-zinc-100 mb-6">{heroData.location}</p>
            
            <div className="flex items-center gap-3 bg-zinc-900/50 p-3 rounded-xl border border-zinc-800/50 w-max">
          ...
          <TiltCard className="md:col-span-1 glass rounded-[2rem] p-8 border border-zinc-800/60 flex flex-col justify-between group overflow-hidden relative cursor-pointer" onClick={() => document.getElementById('terminal')?.scrollIntoView({ behavior: 'smooth' })}>
            ...
            <div className="flex justify-between items-start mb-8">
              ...
              <ArrowRight size={20} className="text-zinc-600 group-hover:text-zinc-100 group-hover:translate-x-1 -rotate-45 group-hover:rotate-0 transition-all duration-300" />
            </div>
            <div>
              <h3 className="text-xl font-bold font-display text-zinc-100 mb-2">Interactive Mode</h3>
              <p className="text-sm text-zinc-500">Ask my AI agent anything in the terminal.</p>
            </div>
          </TiltCard>
