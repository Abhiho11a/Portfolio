...
function ImageGallery({ images, color }) {
  ...
  return (
    <div className="relative rounded-xl overflow-hidden border border-zinc-800/50">
      <div className="relative aspect-video bg-zinc-900/80 flex items-center justify-center">
        <AnimatePresence mode="wait">
          <motion.div key={current} initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0 }} transition={{ duration: 0.3 }} className="absolute inset-0 flex flex-col items-center justify-center p-6">
            ...
            <span className="text-sm text-zinc-500 text-center">{images[current].placeholder}</span>
            <span className="text-xs font-medium mt-2 px-3 py-1 rounded-full" style={{ color, background: `${color}12` }}>{images[current].label}</span>
          </motion.div>
        </AnimatePresence>
        {images.length > 1 && (<>
          <button onClick={() => setCurrent((p) => (p === 0 ? images.length - 1 : p - 1))} className="absolute left-2 top-1/2 -translate-y-1/2 w-8 h-8 rounded-full glass flex items-center justify-center text-zinc-400 hover:text-white transition-colors"><ChevronLeft size={16} /></button>
          <button onClick={() => setCurrent((p) => (p === images.length - 1 ? 0 : p + 1))} className="absolute right-2 top-1/2 -translate-y-1/2 w-8 h-8 rounded-full glass flex items-center justify-center text-zinc-400 hover:text-white transition-colors"><ChevronRight size={16} /></button>
        </>)}
      </div>
      <div className="flex items-center justify-center gap-1.5 py-2 bg-zinc-950/50">
...
function HackathonCard({ item, index }) {
  ...
  return (
    <TiltCard>
      <motion.div initial={{ opacity: 0, y: 40 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, margin: '-60px' }} transition={{ duration: 0.6, delay: index * 0.15 }} className="glass rounded-2xl border border-zinc-800/60 hover:border-zinc-700/80 transition-all duration-300 overflow-hidden">
      <div className="h-1 w-full" style={{ background: `linear-gradient(90deg, ${item.color}, transparent)` }} />
      <div className="p-6 md:p-8">
        <div className="flex items-start justify-between mb-4">
          <div className="flex-1">
            <div className="flex items-center gap-2 mb-2"><span className="text-2xl">{item.trophy}</span><h3 className="text-xl font-bold text-zinc-100">{item.title}</h3></div>
        ...
        <div className="mb-5">
          <h4 className="text-sm font-semibold text-zinc-200 mb-2 flex items-center gap-2"><Flame size={14} style={{ color: item.color }} />What We Built</h4>
          <ul className="space-y-1.5">{item.whatWeBuilt?.map((point, i) => (<li key={i} className="flex items-start gap-2 text-sm text-zinc-400"><span className="w-1 h-1 rounded-full mt-2 flex-shrink-0" style={{ background: item.color }} />{point}</li>))}</ul>
        </div>
        ...
        <div className="flex flex-wrap gap-3 pt-4 border-t border-zinc-800/50">
          ...
          {item.github && (<a href={item.github} target="_blank" rel="noopener noreferrer" className="flex items-center gap-2 px-4 py-2 rounded-lg text-sm font-medium bg-zinc-900/60 border border-zinc-800/50 text-zinc-300 hover:text-cyber-green hover:border-[var(--accent)]/30 transition-all"><GitFork size={14} />GitHub Repo</a>)}
        </div>
      </div>
      </motion.div>
    </TiltCard>
...
function EventCard({ item, index }) {
  return (
    <TiltCard>
      <motion.div initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.5, delay: index * 0.1 }} whileHover={{ x: 5, transition: { duration: 0.2 } }} className="glass rounded-xl p-5 md:p-6 border border-zinc-800/60 hover:border-zinc-700/80 transition-all group">
      <div className="flex items-start gap-4">
        ...
        <div className="flex-1 min-w-0">
          ...
          <p className="text-sm text-zinc-400 leading-relaxed mb-4">{item.experience}</p>
          <div>
            <h4 className="text-xs font-semibold text-zinc-300 mb-2 flex items-center gap-1.5"><MessageCircle size={12} style={{ color: item.color }} /> Key Takeaways</h4>
    ...
    <section id="achievements" className="relative py-24 px-6">
      ...
      <div className="relative max-w-5xl mx-auto">
        <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} className="mb-10">
          ...
          <h2 className="text-3xl md:text-4xl font-bold text-zinc-100">Hackathons, Events & <span className="text-gradient-warm">Wins</span></h2>
          <p className="text-zinc-500 mt-2 max-w-lg">Building under pressure, learning from the community, always showing up.</p>
        </motion.div>
        ...
        <div className="flex gap-2 mb-8 overflow-x-auto pb-2">
          {tabs.map((tab) => {
            ...
            return (
              <button key={tab.id} onClick={() => setActiveTab(tab.id)} className={`relative flex items-center gap-2 px-5 py-2.5 rounded-xl text-sm font-medium transition-all flex-shrink-0 ${isActive ? 'text-zinc-100' : 'text-zinc-500 hover:text-zinc-300 hover:bg-zinc-900/40'}`}>
                {isActive && (<motion.div layoutId="achievementTabBg" className="absolute inset-0 glass rounded-xl border border-zinc-700/50" style={{ boxShadow: `0 0 15px ${tab.color}10` }} transition={{ type: 'spring', stiffness: 400, damping: 30 }} />)}
