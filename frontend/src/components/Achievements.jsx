      ...
      <div className="relative aspect-video bg-zinc-50/80 dark:bg-zinc-900/80 flex items-center justify-center">
        <AnimatePresence mode="wait">
          <motion.div key={current} initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0 }} transition={{ duration: 0.3 }} className="absolute inset-0 flex flex-col items-center justify-center p-6">
            ...
            <span className="text-sm text-zinc-500 dark:text-zinc-500 dark:text-zinc-500 dark:text-zinc-500 text-center">{images[current].placeholder}</span>
    ...
    <TiltCard>
      ...
      <div className="flex items-start gap-4">
        ...
        <div className="flex-1 min-w-0">
          ...
          <div className="flex flex-wrap items-center gap-3 text-xs text-zinc-500 dark:text-zinc-500 dark:text-zinc-500 dark:text-zinc-500 mb-3">
          ...
          <div>
            ...
            <ul className="space-y-1">
              {item.takeaways?.map((t, i) => (
                <li key={i} className="flex items-start gap-2 text-xs text-zinc-500 dark:text-zinc-500 dark:text-zinc-500 dark:text-zinc-500">
    ...
    <section id="achievements" className="relative py-24 px-6">
      ...
      <div className="relative max-w-5xl mx-auto">
        <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} className="mb-10">
          ...
          <p className="text-zinc-500 dark:text-zinc-500 dark:text-zinc-500 dark:text-zinc-500 mt-2 max-w-lg">Building under pressure, learning from the community, always showing up.</p>
        </motion.div>
        ...
        <div className="flex gap-2 mb-8 overflow-x-auto pb-2">
          {tabs.map((tab) => {
            ...
            return (
              <button key={tab.id} onClick={() => setActiveTab(tab.id)} className={`relative flex items-center gap-2 px-5 py-2.5 rounded-xl text-sm font-medium transition-all flex-shrink-0 ${isActive ? 'text-zinc-900 dark:text-zinc-100' : 'text-zinc-500 dark:text-zinc-500 dark:text-zinc-500 dark:text-zinc-500 hover:text-zinc-700 dark:hover:text-zinc-300 hover:bg-zinc-50/40 dark:hover:bg-zinc-900/40'}`}>
